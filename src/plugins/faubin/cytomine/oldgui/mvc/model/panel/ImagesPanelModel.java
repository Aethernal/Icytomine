...
public class ImagesPanelModel extends Model {
	...
	public void generateAndUploadSectionROI(ImageInstanceCollection images) throws CytomineException {
		...
		processFrame.println("Generating sections and upload to cytomine for " + images.size() + " images");
		...
		for (int i = 0; i < images.size(); i++) {
			...
			IcytomineUtil.generateSectionsROI(cytomine, job, controller.getProjectID(),instance, sequence, processFrame);
	...
	public void deleteAllAnnotations(ImageInstanceCollection images) {
		...
		for (int i = 0; i < images.size(); i++) {
			...
			try {
				int nb = IcytomineUtil.deleteAllAnnotation(cytomine, image,
						processFrame);
