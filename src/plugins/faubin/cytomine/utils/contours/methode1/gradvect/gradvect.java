...
public class gradvect {
		...
		for(i=1; i< L-1; i++){ 
		   for(j=1; j< C-1; j++){
		    ...
		    /*grad=(p+q)/250;*/ /*gradient marginal (module des modules des trois grad)*/
		     ...
		     grad=(float) (Math.sqrt(0.5*(p+q+Math.sqrt(((p+q)*(p+q))-4.0*(p*q-t*t)))));  /* gradient de DI ZENZO*/
	...
	public static int gradvc_space(int[] i2din, int[] i2din2, int[] i2din3, i2d_t i2dout, int C, int L){
...
//		 ptrrout=(i2d_u8_t[])I2DGetImagePtr(i2dout);
		 ...
		 for(i=1; i< L-1; i++)
		   for(j=1; j< C-1; j++){
		     ...
		     grad=(Math.sqrt(0.5*(p+q+Math.sqrt(((p+q)*(p+q))-4.0*(p*q-t*t)))));  /* gradient de DI ZENZO*/
